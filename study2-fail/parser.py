...
def word_output(i, std_word, word, is_undo):
	...
	if technique == 'normal':
		...
		return technique + ', ' + str(keyboard_size) + ', ' + str(cursor_speed) + ', ' + std_word + ', ' + str(len(std_word)) + ', ' + word_class + ', ' + correct + ', ' + str(gesture_durations) + ', ' + str(selection_durations) + ', ' + str(sample_distance) + ', ' + str(start_distance) + ', ' + str(end_distance) + ', ' + str(gesture_length) + ', ' + str(gesture_speed) + '\n'
...
oup.write('user, technique, size, speed, session, index, phrase, rate, error, undo\n')

if technique == 'normal':
	woup = file('gesture_' + file_name + '.txt', 'w')
	woup.write('technique, size, speed, word, len, class, correct, gesture_durations, selection_durations, sample_distance, start_distance, end_distance, gesture_length, gesture_speed\n')
...
for i in range(0, word_cnt):
	...
	if i == word_cnt - 1 or phrase_start[i + 1] == 1:
		...
		oup.write(userName + ', ' + technique + ', ' + str(keyboard_size) + ', ' + str(cursor_speed) + ', ' + str(session_index) + ', ' + str(phrase_index) + ', ' + phrase + ', ' + str(rate) + ', ' + str(err) + ', ' + is_undo + '\n')
		letter_cnt = 0
		total_time = 0
		phrase = ''
